...
# Get IP address for a machine with a specific label
# Args:
#   $1: Label name to search for (e.g. "dns-primary")
# Returns:
#   IP address of the first machine with the specified label set to true
#   Exits with error if no machine has the label
machines_get_ip_by_label() {
    local label="$1"
    local machines_file="${MACHINES_FILE:-$PROJECT_ROOT/machines.yaml}"
    ...
    for machine_key in "${machine_keys[@]}"; do
        local has_label
        has_label=$(yq ".machines.\"${machine_key}\".labels.\"${label}\" // false" "$machines_file" | tr -d '"')
