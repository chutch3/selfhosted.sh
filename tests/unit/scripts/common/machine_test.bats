...
setup() {
    # Load test helper functions moved to top level
...
@test "ssh functions should be available after sourcing ssh.sh" {
    ...
    run bash -c "source scripts/common/ssh.sh 2>/dev/null && type ssh_key_auth"
    echo "Output: $output" >&2
    [ "$status" -eq 0 ]
...
@test "machines_get_ip_by_label should return IP of machine with specified label" {
    cat > "${TEST_TEMP_DIR}/machines.yaml" <<EOF
machines:
  primary-node:
    ip: 192.168.1.100
...
EOF
    ...
    run machines_get_ip_by_label "dns-primary"
    [ "$status" -eq 0 ]
    [ "$output" = "192.168.1.100" ]
}

@test "machines_get_ip_by_label should return failure when label not found" {
    cat > "${TEST_TEMP_DIR}/machines.yaml" <<EOF
machines:
  node1:
    ip: 192.168.1.100
...
EOF
    ...
    run machines_get_ip_by_label "nonexistent-label"
    [ "$status" -eq 1 ]
}
