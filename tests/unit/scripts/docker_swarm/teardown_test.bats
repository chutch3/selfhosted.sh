...
EOF
    ...
    mkdir -p "${TEST_TEMP_DIR}/scripts/common"
    cat > "${TEST_TEMP_DIR}/scripts/common/dns.sh" <<'EOF'
#!/bin/bash
get_dns_token() {
...
EOF
    ...
    # Create mock ssh.sh
    cat > "${TEST_TEMP_DIR}/scripts/common/ssh.sh" <<'EOF'
#!/bin/bash
ssh_key_auth() {
    echo "Mock SSH: $*" >&2
    return 0
...
EOF
    ...
    source "${BATS_TEST_DIRNAME}/../../../../scripts/docker_swarm/teardown.sh" || true
...
@test "nuke script should source DNS functions if available" {
    ...
    [ -f "$PROJECT_ROOT/scripts/common/dns.sh" ]

    # Verify get_dns_token function is available after sourcing
    run bash -c "source '$PROJECT_ROOT/scripts/common/dns.sh' && type get_dns_token"
    [ "$status" -eq 0 ]
...
EOF
    ...
    source "${BATS_TEST_DIRNAME}/../../../../scripts/common/machine.sh"
...
@test "nuke should use IP addresses not hostnames for node connections" {
    ...
    run grep -n "machines_get_ip" "${BATS_TEST_DIRNAME}/../../../../scripts/docker_swarm/teardown.sh"
    [ "$status" -eq 0 ]
    [[ "$output" == *"machine_ip=\$(machines_get_ip"* ]]

    # Check that it uses machine_ip variable in NODES array
    run grep -A2 -B2 "NODES+=.*machine_ip" "${BATS_TEST_DIRNAME}/../../../../scripts/docker_swarm/teardown.sh"
    [ "$status" -eq 0 ]
    [[ "$output" == *"\$ssh_user@\$machine_ip"* ]]

    # Verify old hostname-based approach is not present
    run grep "machines_build_hostname" "${BATS_TEST_DIRNAME}/../../../../scripts/docker_swarm/teardown.sh"
    [ "$status" -eq 1 ]  # Should not be found
}

@test "init_machines_config should not produce empty hostnames" {
...
EOF
    ...
    run bash -c "
        ...
        cp '${BATS_TEST_DIRNAME}/../../../../scripts/common/machine.sh' scripts/common/
        cp '${BATS_TEST_DIRNAME}/../../../../scripts/common/ssh.sh' scripts/common/

        # Source the functions and test init_machines_config
        source scripts/common/machine.sh
        source '${BATS_TEST_DIRNAME}/../../../../scripts/docker_swarm/teardown.sh'
...
# Tests for stack-specific teardown functionality

@test "discover_stack_volumes_on_node should find volumes on a specific node" {
    # Mock ssh_key_auth to return volumes for the stack
    ssh_key_auth() {
        local node="$1"
        local cmd="$2"

        # Simulate finding volumes on this node
        if [[ "$cmd" == *"volume ls"* ]]; then
            echo "sonarr_config"
            echo "sonarr_torrents"
            echo "sonarr_usenet"
        fi
        return 0
    }
    ...
    run discover_stack_volumes_on_node "user@192.168.1.100" "sonarr"
    [ "$status" -eq 0 ]
    [[ "$output" == *"sonarr_config"* ]]
    [[ "$output" == *"sonarr_torrents"* ]]
    [[ "$output" == *"sonarr_usenet"* ]]
}

@test "teardown_stack should remove stack and clean volumes on all nodes" {
    docker() {
        case "$1 $2" in
            "stack rm")
                echo "DOCKER_CMD: stack rm $3" >&2
                return 0
                ;;
        esac
    }
    ...
    ssh_key_auth() {
        local node="$1"
        local cmd="$2"
        echo "SSH_CMD: $node: $cmd" >&2

        # Simulate finding volumes on each node
        if [[ "$cmd" == *"volume ls"* ]]; then
            if [[ "$node" == *"192.168.1.100"* ]]; then
                echo "sonarr_config"
                echo "sonarr_torrents"
            elif [[ "$node" == *"192.168.1.101"* ]]; then
                echo "sonarr_config"  # Orphaned volume on old node
            fi
        fi
        return 0
    }

    export -f docker ssh_key_auth
    export NODES=("user@192.168.1.100" "user@192.168.1.101")

    run --separate-stderr teardown_stack "sonarr" false
    [ "$status" -eq 0 ]

    # Verify stack was removed (check output since docker stderr is redirected to /dev/null)
    [[ "$output" == *"Removing stack: sonarr"* ]]
    [[ "$output" == *"Stack teardown completed: sonarr"* ]]

    # Verify volumes were discovered and cleaned on all nodes (check stderr for SSH commands)
    [[ "$stderr" == *"SSH_CMD: user@192.168.1.100: docker volume ls"* ]]
    [[ "$stderr" == *"SSH_CMD: user@192.168.1.101: docker volume ls"* ]]
    [[ "$stderr" == *"SSH_CMD: user@192.168.1.100: docker volume rm sonarr_config"* ]]
    [[ "$stderr" == *"SSH_CMD: user@192.168.1.101: docker volume rm sonarr_config"* ]]
}

@test "teardown_stack with --dry-run should not remove anything" {
    docker() {
        echo "DOCKER_CMD: $*" >&2
        return 0
    }
    ...
    ssh_key_auth() {
        ...
        # Simulate finding volumes on nodes
        if [[ "$cmd" == *"volume ls"* ]]; then
            echo "sonarr_config"
            echo "sonarr_torrents"
        elif [[ "$cmd" == *"volume inspect"* ]]; then
    ...
    run --separate-stderr teardown_stack "sonarr" true
    [ "$status" -eq 0 ]
    [[ "$output" == *"DRY RUN"* ]]
    [[ "$output" == *"Would remove"* ]]

    # Should have called volume ls and volume inspect but NOT stack rm or volume rm
    [[ "$stderr" == *"SSH_CMD:"*"volume ls"* ]]
    [[ "$stderr" != *"DOCKER_CMD: stack rm"* ]]
    [[ "$stderr" != *"volume rm"* ]]
}

@test "main should accept --stack option and call teardown_stack" {
    # Mock teardown_stack to verify it gets called
    teardown_stack() {
        echo "TEARDOWN_STACK_CALLED: $1 $2" >&2
        return 0
    }

    # Mock init_machines_config
    init_machines_config() {
        export NODES=("user@192.168.1.100")
        return 0
    }

    export -f teardown_stack init_machines_config

    run --separate-stderr main --stack sonarr
    [ "$status" -eq 0 ]

    # Verify teardown_stack was called with correct arguments
    [[ "$stderr" == *"TEARDOWN_STACK_CALLED: sonarr false"* ]]
}

@test "main should accept --stack with --dry-run option" {
    # Mock teardown_stack to verify it gets called with dry_run=true
    teardown_stack() {
        echo "TEARDOWN_STACK_CALLED: $1 $2" >&2
        return 0
    }
    ...
    run --separate-stderr main --stack sonarr --dry-run
    [ "$status" -eq 0 ]

    # Verify teardown_stack was called with dry_run=true
    [[ "$stderr" == *"TEARDOWN_STACK_CALLED: sonarr true"* ]]
}

@test "main should error if --dry-run is used without --stack" {
    # Mock init_machines_config
    init_machines_config() {
        export NODES=("user@192.168.1.100")
        return 0
    ...
    export -f init_machines_config

    run main --dry-run
    [ "$status" -eq 1 ]
    [[ "$output" == *"--dry-run is only supported with --stack"* ]]
}
